...
const GenresPage = (props) => {
    ...
    const {setLoading, trending, setTrending, loading, setGenreId} = useContext(MainContext);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const genreList = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${key}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=${genreId}`);
            setLoading(true);
